...
import ExpenditureCategoryService from './services/ExpenditureCategoryService';
...
const ExpenditureCategories = () => {
  ...
  const fetch = async (year = null) => {
    ...
    try {
      ...
      const res = await ExpenditureCategoryService.getCategories(params);
      setList(res.data || []);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = (id) => {
    Modal.confirm({
      ...
      async onOk() {
        try {
          await ExpenditureCategoryService.deleteCategory(id);
  ...
  const beforeUpload = (file) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      message.error('Invalid image type. Allowed: jpg, png, webp, svg');
      return false;
    }
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (editing) {
        await ExpenditureCategoryService.updateCategory(editing.id, payload, selectedFile);
        message.success('Updated');
      } else {
        await ExpenditureCategoryService.createCategory(payload, selectedFile);
  ...
  return (
    <div style={{padding:16}}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:12, gap:12}}>
          <h3 style={{margin:0}}>Expenditure Categories</h3>
      ...
      <Modal title={editing ? 'Edit Category' : 'Create Category'} open={isModalOpen} onCancel={()=>{setIsModalOpen(false); form.resetFields(); setSelectedFile(null); if (previewUrl && previewUrl.startsWith && previewUrl.startsWith('blob:')) { try { URL.revokeObjectURL(previewUrl); } catch(e){} } setPreviewUrl(null);}} footer={null}>
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item label="Icon">
            <input type="file" accept="image/*" onChange={(e) => { const f = e.target.files && e.target.files[0]; if (f) { const allowed = ['image/jpeg','image/png','image/webp','image/svg+xml']; if (!allowed.includes(f.type)) { message.error('Invalid image type'); return; } if (f.size > 5*1024*1024) { message.error('Image too large'); return; } setSelectedFile(f); setPreviewUrl(URL.createObjectURL(f)); } }} />
      ...
      <Modal title="Category" open={isViewOpen} onCancel={()=>setIsViewOpen(false)} footer={[<Button key="close" onClick={()=>setIsViewOpen(false)}>Close</Button>]}> 
        {viewing && (
          <div>
