...
import { Table, Button, Select, message, Space, Modal, Form, Input } from 'antd';
import { PlusOutlined } from '@ant-design/icons';
import IncomePeriodService from './services/IncomePeriodService';
...
const IncomePeriods = () => {
  ...
  const fetch = async (year = null) => {
    ...
    try {
      ...
      const res = await IncomePeriodService.getIncomePeriods(params);
      setList(res.data || []);
      setLastDebug({ params, count: (res.data || []).length });
    } catch (err) {
      console.error(err);
      message.error('Failed to load income periods');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onFinish = async (values) => {
    ...
    setLoading(true);
    try {
      await IncomePeriodService.createIncomePeriod(payload);
  ...
  return (
    <div style={{padding:16}}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:12, gap:12}}>
        <h3 style={{margin:0}}>Income Periods</h3>
      ...
      <Modal title="Create Income Period" open={isModalOpen} onCancel={()=>{setIsModalOpen(false); form.resetFields();}} footer={null}>
