...
const Officers = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (editing) {
        await OfficerService.updateOfficer(editing.id, payload, selectedFile);
        message.success('Updated');
      } else {
        await OfficerService.createOfficer(payload, selectedFile);
  ...
  return (
    <div style={{padding:16}}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:12, gap:12}}>
        <h3 style={{margin:0}}>Office Bearers / Officers</h3>
      ...
      <Modal title={editing ? 'Edit Officer' : 'Create Officer'} open={isModalOpen} onCancel={()=>{setIsModalOpen(false); form.resetFields(); setSelectedFile(null); if (previewUrl && previewUrl.startsWith && previewUrl.startsWith('blob:')) { try { URL.revokeObjectURL(previewUrl); } catch(e){} } setPreviewUrl(null);}} footer={null}>
      ...
      <Modal title="Officer" open={isViewOpen} onCancel={()=>setIsViewOpen(false)} footer={[<Button key="close" onClick={()=>setIsViewOpen(false)}>Close</Button>]}> 
        {viewing && (
          <div>
            <p><strong>Name:</strong> {viewing.name}</p>
            <p><strong>Designation:</strong> {viewing.designation}</p>
            {viewing.image && <p><strong>Image:</strong><br/><img src={viewing.image} alt="officer" style={{maxWidth:200}}/></p>}
